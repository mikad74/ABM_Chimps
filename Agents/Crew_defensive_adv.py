...
class Chimp_crew(Agent):
    ...
    def conflict(self, other_crew, oasis, cF=20, cL=10, cI=5, cR=0):
        ...
        def probs(t_att, t_def):
            if t_att > t_def:
                return 1.0, 0.0, 0.0  # dominate
            if t_att < t_def:
                return 0.0, 1.0, 0.0  # intimidated
            return 0.0, 0.0, 1.0  # tie, leads to fight

        pd, pi, pt = (
            probs(t_self, t_other) if self_is_attacker else probs(t_other, t_self)
        )
    ...
    def move(self, grid_length, oases, crews, motion_accuracy=100):
        neighbourhood = [
            ...
            if 0 <= self.X + di < grid_length and 0 <= self.Y + dj < grid_length
        ]
        # here check for neighboring crews to avoid collision
        available_nbh = [
            pos for pos in neighbourhood if pos not in [crew.pos for crew in crews]
        ]

        if len(available_nbh) > 1 and oases:
            # based on global knowledge of all oases, we pick the closest
            accessible_oases = [
                oasis for oasis in oases if oasis.id not in self.unaccessible_oases
            ]
            if accessible_oases:
                closest_oasis = min(
                    accessible_oases,
                    key=lambda oasis: euclidean_distance(oasis.pos, self.pos),
                )

                distances2oasis = [
                    euclidean_distance(closest_oasis.pos, pos) for pos in available_nbh
                ]

                # the least the distance the higher the chance to be picked as next pos
                weights_ = [
                    (1 - (dist / sum(distances2oasis))) ** motion_accuracy
                    for dist in distances2oasis
                ]
