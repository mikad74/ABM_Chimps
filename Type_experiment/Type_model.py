...
class Type_Model(Model):
    def __init__(
        ...
        super().__init__(
            n_crews,
            grid_size,
            initialise_crews=False,
            oasis_spawn_proportional=oasis_spawn_proportional,
            abundance_factor=abundance_factor,
            oasis_density=oasis_density,
            food_consumption_speed=food_consumption_speed,
        )
        self.crews = {}
    ...
    def add_chimp_crew(self, pos=None, strat=None):
        ...
        id = next(self.id_gen)
        new_crew = Type_Chimp_crew(
            id, pos, strat=strat, consumption_rate=self.crew_consumption_rate
        )
    ...
    def run(self, prob_win=1 / 2):
        ...
        for crew in self.crews.values():  # TODO: Randomize order??
            ...
            if not crew.oasis:
                ...
                for oasis in self.oases.values():
                    if (
                        abs(crew.X - oasis.X) <= 1
                        and abs(crew.Y - oasis.Y) <= 1
                        and oasis.id not in crew.unaccessible_oases
                    ):
                        if oasis.occupied:
                ...
                # Occupied oasis => Play game
                    ...
                    other_crew = [
                        crew for crew in self.crews.values() if crew.pos == oasis.pos
                    ][0]
                    ...
                    if (
                        crew.type == 0 or crew.type < other_crew.type
                    ):  # crew gets intimidated out by the defending crew
