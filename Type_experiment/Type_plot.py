...
for idx, resource in enumerate(tqdm(resource_values)):
    data_track = []
    data_track_oases = []
    food_per_chimp_ = []
    ...
    for i in range(n_sim):
        food_per_chimp = []
        model = Model(10 * n_types, 100, n_types, 20, resource=resource)
    ...
    axs[idx].set_title(f'resource per oasis = {resource}')
