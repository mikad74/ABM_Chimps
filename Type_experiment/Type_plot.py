...
for idx, abundance in enumerate(tqdm(abundance_values)):
    ...
    for i in range(n_sim):
        ...
        for j in range(sim_length):
            ...
            if [crew for crew in model.crews.values()]:
                food_per_chimp.append(
                    sum([oasis.resource for oasis in model.oases.values()])
                    / (100 * sum([crew.crew_size for crew in model.crews.values()]))
                )
            else:
    ...
    axs[idx].plot(
        t,
        food_per_chimp_overtime,
        label="Food per Chimp (mean)",
        linestyle=":",
        color="orange",
    )
